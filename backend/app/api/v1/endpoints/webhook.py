...
class TradeSignal(BaseModel):
    ...
    def validate_size(cls, v: Optional[str]) -> Optional[str]:
        if v is not None:
            try:
                size_val = float(v)
                if not (0 < size_val <= 100):
                    raise ValidationError("Size must be between 0 and 100", context={"size": v})
            except ValueError:
                raise ValidationError("Invalid size format", context={"size": v})
        return v
    ...
    def check_required_fields_and_consistency(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if order_type in {SignalOrderType.LONG_SIGNAL, SignalOrderType.SHORT_SIGNAL}:
            required_fields = ["side", "size", "leverage"]
        elif order_type in {SignalOrderType.LONG_LADDER, SignalOrderType.SHORT_LADDER}:
            required_fields = ["side", "size", "leverage", "takeprofit"]
        else:
            required_fields = []
...
async def tradingview_webhook(
    ...
    logger.info("Received webhook request", extra={"headers": dict(request.headers), **context})
