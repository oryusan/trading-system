...
- **Signal vs. Ladder Processing:**  
  ...
  - **Signal Orders:**  
    ...
    - This includes checking for existing positions, setting leverage, calculating position size, and placing a standard trade order.
  - **Ladder Orders:**  
...
- **Order Placement:**  
  - Place a new trade order on the exchange using the normalized symbol, calculated position size, and current market price (retrieved via the exchange’s ticker API).  
